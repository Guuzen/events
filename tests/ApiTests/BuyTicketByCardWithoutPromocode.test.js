...
const fondy = new Fondy(eventsHttpClient);
...
describe('Buy silver ticket by wire without promocode', function () {
    ...
    test('visitor pay order by card', async () => {
        const response = await visitor.payOrderByCard({
            orderId: orderId
        });
        expect(response).toMatchSnapshot(
            responseSnapshotTemplate()
    ...
    test('fondy mark order paid', async () => {
        const response = await fondy.markOrderPaid({
            orderId: orderId,
        });
        expect(response).toEqual(
            responseTemplate([])
